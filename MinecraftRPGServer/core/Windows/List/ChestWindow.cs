public sealed class ChestWindow : AbstractWindow
{
    ...
    public override Item GetItem(int index)
    {
        ...
        if (index >= 27 && index <= 53) return player.mainInv[index - 27];
        if (index >= 54 && index <= 52) return player.hotbar[index - 54];
        return null;
    }
    public override void SetSlot(int index, Item newItem)
    {
        void f()
        {
            ...
            if (index >= 27 && index <= 53) { player.mainInv[index - 27] = newItem; return; }
            if (index >= 54 && index <= 52) { player.hotbar[index - 54] = newItem; return; }
        }
